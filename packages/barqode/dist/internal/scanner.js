 ...
 */
async function createBarcodeDetector(formats) {
    ...
    if (unsupportedFormats.length > 0) {
        console.debug(`[barqode] native BarcodeDetector does not support formats ${JSON.stringify(unsupportedFormats)}. Will use polyfill.`);
        return new BarcodeDetector({ formats });
    }
 ...
 * interrupted.
 */
export async function setScanningFormats(formats) {
 ...
 */
export async function keepScanning(videoElement, { detectHandler, locateHandler, minDelay, formats }) {
...
export async function processFile(file, formats = ["qr_code"]) {
...
export async function processUrl(url, formats = ["qr_code"]) {
    // To scan files/urls we use one-off `BarcodeDetector` instances,
    // since we don't scan as often as camera frames. Note, that we
    // always use the polyfill. This is because (at the time of writing)
    // some browser/OS combinations don't support `Blob`/`File` inputs
