...
import { StreamApiNotSupportedError, InsecureContextError, StreamLoadTimeoutError, } from "./errors.js";
import { eventOn, sleep } from "./callforth.js";
import shimGetUserMedia from "./shim-get-user-media.js";
import { assertNever } from "./util.js";
let taskQueue = Promise.resolve({ type: "stop", data: null });
/**
 ...
 */
async function runStartTask(video, constraints, torch) {
 ...
 */
export async function start(video, { constraints, torch, restart = false, }) {
    ...
    taskQueue = taskQueue
        ...
        if (prevTaskResult.type === "start") {
            ...
            const { data: { video: prevVideo, stream: prevStream, constraints: prevConstraints, isTorchOn: prevIsTorchOn, }, } = prevTaskResult;
            ...
            return runStopTask(prevVideo, prevStream, prevIsTorchOn).then(() => runStartTask(video, constraints, torch));
        }
 ...
 */
async function runStopTask(video, stream, isTorchOn) {
...
export async function stop() {
    ...
    taskQueue = taskQueue.then((prevTaskResult) => {
        ...
        const { data: { video, stream, isTorchOn }, } = prevTaskResult;
